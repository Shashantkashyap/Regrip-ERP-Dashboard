...
function InspectionFilter({ isVisible, onClose, onSubmit }) {
  ...
  const formData = useSelector((state) => state.inspectionFilter.formData);
  ...
  const filters = [
    ...
    { label: "End Date :", type: "date", name: "end_date", placeholder: "to" },
    {
      label: "Inspection count less than :",
      type: "text",
      name: "manufacturer_name",
      placeholder: "Enter count less than (e.g. 5)",
    },
  ];
  ...
  useEffect(() => {
    ...
    dispatch(setInspectionFormData(initialFormData));
  }, [dispatch]);
  ...
  const handleInputChange = (e) => {
    ...
    dispatch(setInspectionFormData({ ...formData, [name]: value }));
    };
