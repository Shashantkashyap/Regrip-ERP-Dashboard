...
function TyrePurchaseFilter({ isVisible, onClose, onSubmit, setFilterData }) {
  const dispatch = useDispatch();
  const formData = useSelector((state) => state.tyrePurchaseFilter.formData);
  ...
  //     return acc;
  //   }, {});
  //   dispatch(setTyrePurchaseFormData(initialFormData));
  // }, [dispatch]);
              ...
              <div key={index} className="mb-4">
                 {filter.type === "date" && (
                    <label className="mb-2 text-sm font-medium">
                      {filter.name === "from" ? "From" : filter.name === "to" ? "To" : ""}
                    </label>
                  )}
                  <input
                    type={filter.type}
...
export default TyrePurchaseFilter;
